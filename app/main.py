...
async def get_weather(request: Request, zip_code: str = Form(...)):
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            current_time = datetime.fromtimestamp(weather_data.get('dt', 0), tz=ZoneInfo('America/New_York'))
            ...
            for item in forecast_items:
                try:
                    ...
                    if forecast_time.date() == today:
                        ...
                        print(f"- {forecast_time.strftime('%Y-%m-%d %I:%M %p')}: different day, skipping")
                        continue
